// internal/service/localcollection/get.go
...
type GetService interface {
	...
	GetPath(ctx context.Context, id string) ([]*localcollection.LocalCollection, error)
}

// getService implements the GetService interface
type getService struct {
	logger      *zap.Logger
	useCase     uc.GetLocalCollectionUseCase
	pathUseCase uc.GetLocalCollectionPathUseCase
}
...
func (s *getService) GetPath(ctx context.Context, id string) ([]*localcollection.LocalCollection, error) {
	// Validate input
	if id == "" {
		s.logger.Error("collection ID is required")
		return nil, errors.NewAppError("collection ID is required", nil)
