// internal/service/filesyncer/upload.go
package filesyncer
...
// UploadToRemoteService defines the interface for uploading local files to remote
type UploadToRemoteService interface {
	Execute(ctx context.Context, input UploadToRemoteInput) (*UploadToRemoteResult, error)
}

// uploadToRemoteService implements the UploadToRemoteService interface
type uploadToRemoteService struct {
	logger                  *zap.Logger
...
// NewUploadToRemoteService creates a new service for uploading to remote
func NewUploadToRemoteService(
	logger *zap.Logger,
	localFileGetUseCase localfileUseCase.GetLocalFileUseCase,
	localFileUpdateUseCase localfileUseCase.UpdateLocalFileUseCase,
	remoteFileCreateUseCase remotefileUseCase.CreateRemoteFileUseCase,
...
func (s *uploadToRemoteService) Execute(
	ctx context.Context,
	...
	// Get the local file
	localFile, err := s.localFileGetUseCase.ByID(ctx, input.LocalID)
	if err != nil {
	...
	if !localFile.RemoteID.IsZero() {
		...
		if err != nil {
			...
			s.logger.Warn("Failed to fetch remote file, will create a new one",
	...
	if remoteFile == nil {
		...
		remoteFile, err = s.remoteFileFetchUseCase.ByID(ctx, remoteFileResponse.ID)
		if err != nil {
			s.logger.Error("Failed to fetch newly created remote file",
