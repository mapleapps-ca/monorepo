// internal/repo/remotefile/auth.go
package remotefile
...
func (r *remoteFileRepository) getAccessToken(ctx context.Context) (string, error) {
	...
	if userData.AccessToken == "" || time.Now().After(userData.AccessTokenExpiryTime) {
		r.logger.Info("Access token is invalid or expired, refreshing",
...
func (r *remoteFileRepository) refreshTokenIfNeeded(ctx context.Context, userData *user.User) error {
