...
func (r *collectionRepository) CreateCollection(ctx context.Context, request *collection.CreateCollectionRequest) (*collection.CollectionResponse, error) {
	r.logger.Debug("Starting CreateCollection process")
	...
	// Parse the response
	r.logger.Debug("Parsing HTTP response body into CollectionResponse")
	var response collection.CollectionResponse
	if err := json.Unmarshal(body, &response); err != nil {
		r.logger.Error("Failed to parse response body into CollectionResponse", zap.ByteString("body", body), zap.Error(err))
