// web/prototyping/maplefile-frontend/src/contexts/AuthContext.jsx
import {
  createContext,
  useContext,
  useEffect,
...
export function AuthProvider({ children }) {
    ...
    const initializeApp = async () => {
      ...
      try {
        ...
        if (loggedIn) {
          ...
          console.log(
            "AuthContext: User is logged in, attempting to load E2EE keys from localStorage.",
          );
          ...
          if (masterKeyB64 && privateKeyB64 && publicKeyB64 && saltB64) {
            try {
              ...
              const loadedMasterKey =
                await cryptoUtils.fromBase64(masterKeyB64);
              const loadedPrivateKey =
                await cryptoUtils.fromBase64(privateKeyB64);
              const loadedPublicKey =
                await cryptoUtils.fromBase64(publicKeyB64);
              ...
              setSalt(loadedSalt);
              console.log(
                "AuthContext: E2EE Keys successfully loaded from localStorage and set in state.",
              );
            } catch (keyLoadError) {
              console.error(
                "AuthContext: Failed to decode/set E2EE keys from localStorage. Clearing them.",
                keyLoadError,
              );
            ...
            console.log(
              "AuthContext: Some or all E2EE Keys not found in localStorage.",
            );
          }
        }
      } catch (error) {
  ...
  const login = useCallback(
    async (
      // Made async to use await for cryptoUtils
      accessToken,
      accessTokenExpiry,
      refreshToken,
      refreshTokenExpiry,
      ...
      try {
        ...
        if (decryptedMasterKey)
          localStorage.setItem(
            "masterKey_e2ee",
            await cryptoUtils.toBase64(decryptedMasterKey),
          );
        if (decryptedPrivateKey)
          localStorage.setItem(
            "privateKey_e2ee",
            await cryptoUtils.toBase64(decryptedPrivateKey),
          );
        if (userPublicKeyBytes)
          localStorage.setItem(
            "publicKey_e2ee",
            await cryptoUtils.toBase64(userPublicKeyBytes),
          );
        if (userSaltBytes)
          localStorage.setItem(
            "salt_e2ee",
            await cryptoUtils.toBase64(userSaltBytes),
          );
