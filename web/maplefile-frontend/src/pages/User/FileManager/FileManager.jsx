...
const FileManager = () => {
  ...
  const { collectionService, fileService, passwordStorageService } = useServices();
  ...
  }, [collections, sharedCollections, collectionsLoading, loadAttempted, hasCollections]);
    ...
    try {
      ...
      if (folder.ancestor_ids && folder.ancestor_ids.length > 0) {
        ...
        for (const ancestorId of folder.ancestor_ids) {
          try {
            const ancestor = await collectionService.getCollection(ancestorId, password);
            crumbs.push({
              id: ancestor.id,
      ...
      setBreadcrumbs([
        ...
        { id: folderId, name: "[Unable to decrypt]", path: `/files/${folderId}` }
      ]);
    }
  ...
  const getDisplayItems = useCallback(() => {
    ...
    if (import.meta.env.DEV) {
      console.log("[FileManager] Display items debug:", {
        ...
        folders: folders.map(f => ({ id: f.id, name: f.displayName, parent_id: f.parent_id })),
    ...
    return items;
  }, [files, getSubfolders, filterType, sortBy, collections, sharedCollections, folderId, collectionsLoading, filesLoading]);
    ...
    return (
      <div style={{ padding: "20px", maxWidth: "600px", margin: "0 auto" }}>
        ...
        <div style={{ backgroundColor: "#f8f9fa", padding: "20px", borderRadius: "4px", marginBottom: "20px" }}>
          <p>Your folders are encrypted. Please enter your password to decrypt them.</p>
          <p style={{ marginBottom: 0, fontSize: "14px", color: "#666" }}>
            Note: You may need to navigate to <a href="/collections" style={{ color: "#007bff" }}>Collections</a> to enter your password first.
          </p>
        </div>
        <button onClick={() => navigate("/collections")} style={{ padding: "10px 20px" }}>
      ...
      <div style={{ padding: "20px", maxWidth: "1400px", margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", padding: "60px", backgroundColor: "#f8f9fa", borderRadius: "8px" }}>
          ...
          <p style={{ color: "#666", marginBottom: "20px" }}>
            Your collections may not have loaded yet, or you may need to create your first collection.
          </p>
    ...
    <div style={{ padding: "20px", maxWidth: "1400px", margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
          <button
            onClick={() => navigate(folderId ? `/collections/create?parent=${folderId}` : "/collections/create")}
            style={{ padding: "8px 16px", backgroundColor: "#007bff", color: "white", border: "none", borderRadius: "4px" }}
          >
            📁 New Folder
          </button>
          <button
            onClick={() => navigate(folderId ? `/collections/${folderId}/add-file` : "/files/upload")}
            style={{ padding: "8px 16px", backgroundColor: "#28a745", color: "white", border: "none", borderRadius: "4px" }}
            disabled={!folderId}
            title={!folderId ? "Select a folder first" : "Upload file to current folder"}
          >
            📄 Upload File
          </button>
          ...
          {import.meta.env.DEV && (
            <button
              onClick={async () => {
                ...
                console.log("[FileManager] Current collections:", { collections, sharedCollections });
                try {
                  const result = await loadAllCollections();
                  console.log("[FileManager] Manual refresh result:", result);
                  console.log("[FileManager] After refresh collections:", { collections, sharedCollections });
                  // Force update
                  forceUpdate({});
                } catch (err) {
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: "10px" }}>
            ...
            >
              {viewMode === "list" ? "📋" : "⚏"} {viewMode === "list" ? "List" : "Grid"}
            </button>
          </div>
        </div>
        ...
        {!isLoading && hasCollections && (
          <div style={{ fontSize: "12px", color: "#666", marginBottom: "10px" }}>
            {collections.length} owned collections, {sharedCollections.length} shared collections loaded
          </div>
        )}
        ...
        {currentFolder && currentFolder.members && currentFolder.members.length > 1 && (
          <div style={{ backgroundColor: "#d4edda", padding: "10px", borderRadius: "4px", marginBottom: "10px" }}>
            <small>Shared with {currentFolder.members.length - 1} user(s)</small>
          </div>
        )}
      </div>
      ...
      {(collectionsError || filesError) && (
        <div style={{ backgroundColor: "#fee", color: "#c00", padding: "15px", marginBottom: "20px", borderRadius: "4px" }}>
          <strong>Error:</strong> {collectionsError || filesError}
          {collectionsError && (
            <div style={{ marginTop: "10px" }}>
              <button
                onClick={() => loadAllCollections()}
                style={{ padding: "5px 10px", backgroundColor: "#c00", color: "white", border: "none", borderRadius: "3px" }}
              >
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "#f8f9fa", borderBottom: "2px solid #dee2e6" }}>
                  ...
                  <th style={{ padding: "12px", textAlign: "left", width: "100px" }}>Size</th>
                  <th style={{ padding: "12px", textAlign: "left", width: "150px" }}>Modified</th>
                  <th style={{ padding: "12px", textAlign: "left", width: "150px" }}>Type</th>
              ...
              <tbody>
                {displayItems.map(item => (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px" }}>{formatFileSize(item.displaySize)}</td>
                    <td style={{ padding: "12px" }}>{formatDate(item.displayDate)}</td>
                    <td style={{ padding: "12px" }}>
                      {item.itemType === "folder" ? "Folder" : item.mime_type || "File"}
                    </td>
                    <td style={{ padding: "12px", textAlign: "center" }} onClick={(e) => e.stopPropagation()}>
                      {item.itemType === "folder" ? (
                        <button
                          onClick={() => handleDeleteFolder(item.id, item.displayName)}
              ...
              {displayItems.map(item => (
                <div
                  ...
                  style={{
                    ...
                    textAlign: "center",
                    cursor: "pointer",
                    backgroundColor: selectedItems.has(item.id) ? "#e9ecef" : "white"
                  }}
                  onClick={() => {
                    if (item.itemType === "folder") {
                      navigate(`/files/${item.id}`);
                ...
                >
                  ...
                  <div style={{ fontSize: "12px", color: "#666", marginTop: "5px" }}>
                    {item.itemType === "folder" ? "Folder" : formatFileSize(item.displaySize)}
                  </div>
                </div>
              ))}
            </div>
      ...
      {selectedItems.size > 0 && (
        <div style={{
          ...
          <button
            ...
            style={{ marginLeft: "20px", padding: "5px 15px", backgroundColor: "#666", color: "white", border: "none", borderRadius: "4px" }}
          >
            Clear
          </button>
        </div>
      ...
      {/* Info box */}
      <div style={{ marginTop: "40px", padding: "20px", backgroundColor: "#f8f9fa", borderRadius: "4px", fontSize: "14px", color: "#666" }}>
        ...
        <ul style={{ marginBottom: 0, paddingLeft: "20px" }}>
          ...
          <li>Access the <a href="/collections" style={{ color: "#007bff" }}>legacy collections view</a> for advanced features</li>
        </ul>
      </div>

      {/* Debug info in development */}
      {import.meta.env.DEV && (
        <details style={{ marginTop: "20px", padding: "10px", backgroundColor: "#f8f9fa" }}>
          ...
          <div>
            ...
            <pre>{JSON.stringify(
              collectionsSample: collections.slice(0, 2).map(c => ({
                ...
                isZeroUUID: c.parent_id === "00000000-0000-0000-0000-000000000000",
            ...
            <pre>{JSON.stringify({
              currentFolderId: folderId || "root",
              getSubfoldersDebug: (() => {
                const allColls = [...new Map([...collections, ...sharedCollections].map(c => [c.id, c])).values()];
                const rootColls = allColls.filter(c => isRootLevel(c.parent_id));
                return {
