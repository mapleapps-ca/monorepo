...
async function refreshTokens(refreshTokenValue, storageData) {
  ...
  try {
    ...
    if (response.status === 201) {
      ...
      if (result.encrypted_tokens) {
        ...
        broadcastMessage("storage_update", {
          ...
          value: "encrypted_" + result.encrypted_tokens,
        });
        broadcastMessage("storage_update", {
      ...
      // Store token nonce if provided
      if (result.token_nonce) {
        broadcastMessage("storage_update", {
          key: "mapleapps_token_nonce",
          value: result.token_nonce,
        });
      }
    ...
    // Also clear token nonce
    broadcastMessage("storage_remove", { key: "mapleapps_token_nonce" });
