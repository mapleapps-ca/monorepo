...
class WorkerManager {
  ...
  sendStorageDataToWorker() {
    const storageData = localStorageService.getCurrentStorageData();
  ...
  async manualRefresh() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.sendToWorker("manual_refresh", {
        ...
        storageData: localStorageService.getCurrentStorageData(),
      });
    });
  }
  ...
  // Redirect to login (placeholder - not implemented in this app)
  redirectToLogin() {
    console.log(
      "[WorkerManager] Would redirect to login page (not implemented in this app)",
    );
  }
